...
const combinedStart = `      } else if (analysisType === 'combined') {`;
const combinedEnd = `      } else {`;

const startIdx = code.indexOf(combinedStart);
const endIdx = code.indexOf(combinedEnd, startIdx);

const newCombinedBlock = `      } else if (analysisType === 'combined') {
...
code = code.substring(0, startIdx) + newCombinedBlock + code.substring(endIdx);
