...
const durationFacts: SecCompanyFact[] = [
  ...
  // Q2 filing contains both standalone-quarter and H1/YTD contexts. The directly reported quarter wins when equally current.
...
assert.equal(normalized[1].derivation, 'reported_standalone');
assert.deepEqual(normalized[1].accessionNumbers, ['q2-quarter']);
...

{
  ...
  assert.equal(values.find(item => item.fiscalQuarter === 3)?.value, 130, 'Current filing-reported Q3 standalone remains authoritative');
...

{
  ...
  assert.equal(values.find(item => item.fiscalQuarter === 3)?.value, 130, 'Reported Q3 standalone is usable without H1 YTD');
