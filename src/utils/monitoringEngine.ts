...
export function convertMaterialEventsToAlerts(
  ...
  for (const event of events) {
    ...
    alerts.push({
      ...
      titleTh: event.headlineTh || event.headline,
      message: event.summaryEn || event.factualSummary,
      messageTh: event.summaryTh || event.factualSummary,
      originalHeadline: event.originalHeadline || event.headline,
      headlineTh: event.headlineTh,
      summaryEn: event.summaryEn,
      summaryTh: event.summaryTh,
      summaryEvidence: event.summaryEvidence,
      timestamp,
      dateStr: alertDate,
      isRead,
      evidence: {
