...
export function PeerComparisonTable({ 
  ...
  // Determine if this peer group has financial metrics (banks/lenders/fintech)
  const isFinancialPeerGroup = peers.some(p => p.pb_ratio !== undefined || p.roe_pct !== undefined);
    ...
    <div className="bg-white rounded-3xl p-5 sm:p-7 border border-stone-200 shadow-sm flex flex-col gap-5 w-full">
      ...
      {!hasPeers ? (
        <div className="p-4 bg-stone-50 rounded-2xl border border-stone-200 text-xs sm:text-sm text-stone-600 leading-relaxed font-sans">
          {isThai
            ? (data.unavailable_message_th || 'ยังไม่พบกลุ่มบริษัทที่เปรียบเทียบได้และมีข้อมูลที่ตรวจสอบแล้วเพียงพอ')
            : (data.unavailable_message_en || 'No sufficiently comparable source-verified peer set is currently available.')}
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="bg-stone-50 border-b border-stone-200 text-stone-700 font-bold uppercase text-xs tracking-wider">
                ...
                {isFinancialPeerGroup ? (
                  <>
                    <th className="py-3 px-3 text-right font-mono min-w-[100px]">{isThai ? 'P/B Ratio' : 'P/B Ratio'}</th>
                    <th className="py-3 px-3 text-right font-mono min-w-[100px]">{isThai ? 'ROE' : 'ROE'}</th>
                  </>
                ) : (
                  <>
                    ...
                    {isFinancialPeerGroup ? (
                      <>
                        <td className="py-3 px-3 text-right text-stone-800">
                          {peer.pb_ratio !== null && peer.pb_ratio !== undefined ? `${peer.pb_ratio}x` : '-'}
                        </td>
                        <td className="py-3 px-3 text-right text-stone-800">
                          {peer.roe_pct !== null && peer.roe_pct !== undefined ? `${peer.roe_pct}%` : '-'}
                        </td>
                      </>
                    ) : (
                      <>
